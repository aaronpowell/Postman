(function() {
  var cache, createCache, deliver, dropByDate, dropByFunction, dropMessages, isArray, isDate, isFunction, receive, retract;
  cache = {};
  ...
  retract = function(name, fn) {
    ...
    if (!fn) {
      return cache[name].subs = [];
    } else {
      subs = cache[name].subs;
      index = subs.indexOf(fn);
      if (index > -1) {
        return subs.splice(0, index).concat(subs.splice(index, subs.length));
      }
    }
  };
